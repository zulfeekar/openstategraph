...
A Dyflow workflow. `workflow.json` in this directory is the source of truth for its nodes and edges — edit it through the editor, not by hand, unless you know the canonical serialization rules (ticket 19: sorted nodes, content-addressed edges, no written edge ids).
