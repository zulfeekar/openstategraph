...
- **Run it**: `POST /api/runs` or `/api/runs/stream` with this directory's `workflow.json` as the `workflow` field, or open `tmp-approval-check` in the editor and use Chat.
