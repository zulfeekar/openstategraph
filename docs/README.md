...
| **stop a right-looking number being wrong** | [Declaring a table](declaring-a-table.md) — the two fields a data source states about itself (`row_key`, `coverage`), why a `COUNT(*)` published under an entity noun is a claim, and why *"0 dark vessels"* over a table that stops in May is not a measurement | [Ports and edges](ports-and-edges.md) |
