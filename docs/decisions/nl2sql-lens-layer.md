...
## `launch-readiness/90` — two descriptions of one warehouse, and no check between them

The lens layer has always had two authors it does not control. The **search
index** (`intelligence.nl2sql_metadata_index`) is what the agent is handed
before its first turn, `usage_hint` and all — and a `usage_hint` is an
instruction, not a description; the agent follows it literally. The **lens
files** are what the validator enforces. Both describe the same warehouse.
Nothing compared them, so a disagreement between the two could only ever be
discovered by losing a conversation to it — which is exactly how
`dim_vessel_latest` was found: three consecutive traces, and a person
reading them.

The doctrine this adds to the file: **when two artifacts describe one
external system, the comparison between them is a test, not a habit.** It
is the same shape as "a rule with no way to fail is a story", pointed at
data instead of at prose.

Swept all three sources pairwise — 87 Unity Catalog tables, 60 index rows
naming 10 tables, 7 lens files — with metadata reads only. One genuine
class-1 gap: `current_vessel_positions_v2r0`, which the index tells the
agent to use for where-is-vessel and fleet-snapshot questions and which no
lens declared, so check 1 rejected every draft that obeyed the hint. Given
its own lens (`vessel_directory`) rather than a dimension row, because it
answers its own question class; declared as a dimension on `cargoflow` only,
because `cargoflow_latest` carries `vessel_imo` and no vessel name and is
therefore where the follow-up "who are those vessels" begins. Zero class-2
disagreements; three class-3 type mismatches, all STRING date columns the
lens files already document as traps, recorded as named exceptions rather
than "fixed" against a warehouse that is the authority.

The deliverable is `tests/test_lenses_match_the_warehouse.py`, which runs
with no warehouse and no network against a committed snapshot
(`data/warehouse_facts.json`, regenerated by
`scripts/refresh_warehouse_facts.py` from Unity Catalog metadata and a
vector-index scan — no statement executed). A table the index names with no
lens, a lens column the warehouse lacks, a declared role contradicting a
real type, and a catalog table in no lens all fail unless a reason is
written down. Silence is not an accepted answer.

Three defects fell out of the sweep, and the second is the one worth
remembering. `workflow.json` embeds a *copy* of every skill file's text on
its `input.markdown` nodes; the files on disk are what the validator reads.
Seven of twelve had drifted, including all four lenses whose
`dim_vessel_latest` join was the original fix — so that fix reached the
validator and never reached the model, along with two routing rules and the
"ask what only the user knows" guidance. **A fix that ships into only one of
two copies has not shipped** (`launch-readiness/91`, pinned by a drift test
in the same file). The `[enumerable]` warning fired on every correct run
because it consulted the index's top-N *sample* about a comparison the
authoritative distinct-values probe answers definitively twelve lines later
— wrong every time, which trains a reader to skip the only block a real
problem appears in (`launch-readiness/92`). And the scaffolded shape test
had been red at every commit since the document outgrew its three-node
scaffold, so the package's suite already exited non-zero before any of this
was added (`launch-readiness/93`).

`cpl-nl2sql` commit `fd3ff2f`, 106 tests passing, no warehouse call. Both
regression gates pass afterward: the Mongstad breakdown (3 attempts, no
