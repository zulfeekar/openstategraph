...
| `openstategraph runs export [--to FILE]` | the same rows as a JSON array, to stdout or to a file. What you run **before** truncating a store that has grown large — nothing here ever deletes a run, because a conversation is the raw material for replay |
...
| `.ask(question, *, thread_id=None, user_email=None, session_id=None, recursion_limit=None, context=None)` | run it once, get a `RunResult`. **`user_email` is who the run is for** — omit it and per-person memory does not bind (see [`deploying.md` §1b](deploying.md)); over HTTP a client may not send it, but here you are the server. `workflow_slug` needs no argument: it comes from the package. **`context` is the other side of that sentence** — identity is who the run is *for* and the server's to decide; `context` is what the workflow *asked its caller for* in `settings.context`, and yours to fill. A plain dict, never a Python type, validated against the document's own declaration before the graph runs: an undeclared key, a required key you omitted, or a value of the wrong declared type is a `RunContextError` naming the key and the workflow |
