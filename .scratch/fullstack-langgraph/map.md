...
- [Tool resolution by node type](issues/33-tool-resolution-by-node-type.md) — built; workflow_slug on all endpoints, resume 422 pinned
- [Subgraph node](issues/34-subgraph-node.md) — v1 built; self-inclusion refused at build time
- [Function nodes](issues/35-function-nodes.md) — v1 built; fn(text)->str contract
- [Workflow settings](issues/36-workflow-settings.md) — round-trips; settings.model is the document default
- [Config surface](issues/38-config-surface.md) — group/advanced + Inspector disclosure built
- [Tabular repair](issues/40-tabular-analytics-repair.md) — live-verified E2E incl. HITL approve/reject; data honesty stays ticket 41
- [Edge direction](issues/44-edge-direction-affordances.md), [flow direction](issues/45-flow-direction-setting.md), [markdown](issues/46-markdown-rendering.md) — built and live-verified
