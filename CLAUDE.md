...
`retry_policy`, `error_handler` and `cache_policy` are parameters of **`StateGraph.add_node`**, available to every node of every family; **`timeout` is one too, and only a node whose body is `async def` can carry it** — LangGraph refuses it at compile time for a synchronous one. `StateGraph.set_node_defaults(...)` accepts all four and applies them graph-wide with per-node override — but this compiler passes it only `retry_policy` and `error_handler`, because a graph-wide `timeout` would be refused by every synchronous node in the document and take the compile with it.

(Until 2026-09-11 this line listed all four as available to every node, and `_node_overrides` duly passed a timeout for every node while the editor offered the field on every card — so a document with a timeout on an Output could not be compiled at all, by anyone, and the only way back was editing the JSON by hand. The library fact was already pinned in `backend/tests/test_a_library_default_is_never_literalised.py`; nothing pinned *our* code against it, which is `langchain-drift-watch` 01 and the reason that map exists. The compiler now drops a timeout it cannot honour and reports it, the editor offers the field only where it works, and the two are held together by `backend/tests/test_the_editor_offers_timeout_only_where_the_compiler_keeps_it.py` rather than by this sentence.) So they live on the **workflow** and compile to graph assembly — never on an agent base, a tool base, or a shared ancestor.
