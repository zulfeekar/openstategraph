...
- **Streaming/observability enhancements**: token streams and per-node traces already flow to the chat panel (ticket 27), and per-node duration now shows in the Inspector for a Chat-driven run too (`AskPanel.tsx`, an honest wall-clock approximation since LangGraph's `updates` stream mode has no start event to measure from). What's left unspecified: a live LangSmith-style trace view or deeper per-node observability on the canvas itself — what would it show that the Inspector's timing badge and the chat panel's activity feed don't already?
