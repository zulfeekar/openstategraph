...
describe('WorkflowSerializer', () => {
  ...
  describe('round trip', () => {
    it('reproduces nodes, edges and name', () => {
      ...
      const outcome = reloaded.controller.importJSON(json);
    ...
    it('preserves node ids, so links still resolve', () => {
      ...
      const reloaded = makeWorkbench();
      reloaded.controller.importJSON(json);
    ...
    it('preserves containment', () => {
      ...
      const reloaded = makeWorkbench();
      reloaded.controller.importJSON(workbench.controller.exportJSON());
    ...
    it('is idempotent — exporting a reloaded document reproduces the bytes', () => {
      seed();
      const first = workbench.controller.exportJSON();

      const reloaded = makeWorkbench();
      reloaded.controller.importJSON(first);
      const second = reloaded.controller.exportJSON();
  ...
  describe('determinism', () => {
    it('produces identical bytes for two independently built identical graphs', () => {
      ...
      workbench = makeWorkbench();
      seed();
      const second = workbench.controller.exportJSON();
    ...
    it('is unchanged by delete-then-undo', () => {
      const { agent } = seed();
      const before = workbench.controller.exportJSON();
      ...
      expect(workbench.controller.exportJSON()).toBe(before);
    });
    ...
    it('is unchanged by the order nodes were created in', () => {
      ...
      connect(workbench, agent, 'result', output, 'result');
      connect(workbench, input, 'text', agent, 'prompt');
      const reversed = workbench.controller.exportJSON();

      workbench = makeWorkbench();
      seed();
      const forward = workbench.controller.exportJSON();
  ...
  describe('canonical order', () => {
    ...
    it('keeps field keys in a stable order however they were edited', () => {
      ...
      workbench.controller.setField(a.id, 'topicLimit', 7);
      workbench.controller.setField(a.id, 'subreddit', 'typescript');
      const first = workbench.controller.exportJSON();

      workbench = makeWorkbench();
      const b = addNode(workbench, TYPE.redditSearch);
      workbench.controller.setField(b.id, 'subreddit', 'typescript');
      workbench.controller.setField(b.id, 'topicLimit', 7);

      expect(workbench.controller.exportJSON()).toBe(first);
    });
  });

  describe('lenient loading', () => {
    it('rejects invalid JSON with a readable message', () => {
      const outcome = workbench.controller.importJSON('{ not json');
      expect(outcome.ok).toBe(false);
      expect(outcome.message).toMatch(/json/i);
    });

    it('rejects a document missing nodes or edges', () => {
      const outcome = workbench.controller.importJSON(JSON.stringify({ version: 1, name: 'x' }));
      expect(outcome.ok).toBe(false);
    });

    it('refuses a document from a newer schema version', () => {
      const outcome = workbench.controller.importJSON(
    ...
    it('skips an unknown node type with a warning rather than failing the load', () => {
      ...
      const reloaded = makeWorkbench();
      const outcome = reloaded.controller.importJSON(JSON.stringify(document));
  ...
  describe('id minting after import', () => {
    it('does not collide a newly added node with an imported one', () => {
      ...
      reloaded.controller.addNode(TYPE.textInput, { x: 0, y: 0 });
