...
import { ALWAYS_OFFERED_OVERRIDE_KEYS } from '../../core/model/ModelRegistry';
...
describe('platform tool nodes', () => {
  ...
  describe('the YouTube transcript atom', () => {
    ...
    it('declares the three keys `configure` reads, with the Python defaults', () => {
      ...
      // The execution-override keys are injected by `ModelRegistry` and read
      // by graph assembly, not by the tool. A tool node's body is synchronous,
      // so it is not offered the timeout — `langchain-drift-watch/02`.
      expect([...fields.keys()]).toEqual([
        'language',
        'allowAutoCaptions',
        'maxChars',
        ...ALWAYS_OFFERED_OVERRIDE_KEYS,
      ]);
