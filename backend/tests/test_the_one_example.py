...
class TestNothingElseStillPointsAtTheDeletedExamples:
    ...
    #: The builders whose topics ARE workflow pointers — they enumerate the
    #: workflows root (`root`) or the project (`project`), and a doc of theirs
    #: whose stem names no package is a pointer into nothing. Every other
    #: builder writes *concept* topics (`sql` per table, `explorer` and
    #: `codebase` per subject), which the architecture explicitly allows into
    #: the same per-package store and whose stems are not slugs at all.
    POINTER_SOURCES = ("root", "project")

    def test_a_pointer_doc_never_routes_nowhere(self) -> None:
        """The great-example-collapse guard, narrowed to what it is about
        (ticket 11).

        It asserted that **every** `concierge/knowledge/*.md` had a matching
        workflow directory. The intent is real — a routing index pointing at a
        workflow that no longer exists is the defect it was written for — but
        as written over `*.md` it asserted something the builder registry is
        *permitted* to violate: `explorer` and `codebase` write concept topics
        into the same store, and their stems are subjects, not slugs. The guard
        would have gone red on a legitimate build.

        Hand-claimed docs have no marker at all (`chinook-assistant.md`, which
        is the never-overwrite contract working) and stay in scope: a claimed
        pointer is still a pointer, and a stale one is exactly as broken.
        """
        from openstategraph.knowledge_builders import marker_source

        concierge = package("concierge")
        checked = 0
        for doc_path in (concierge / "knowledge").glob("*.md"):
            source = marker_source(doc_path.read_text())
            pointer_shaped = source is None or source in ("", *self.POINTER_SOURCES)
            if not pointer_shaped:
                continue
            checked += 1
            assert (concierge.parent / doc_path.stem).is_dir(), f"{doc_path.name} routes nowhere"

        # Anti-vacuity: a narrowing that narrowed to nothing would pass while
        # asserting nothing, which is worse than the over-broad version.
        assert checked >= 2, "the gateway's routing index lost its pointer docs"

    def test_a_concept_topic_is_allowed_in_the_same_store(self) -> None:
        """The behaviour the old guard forbade, stated so the narrowing cannot
        quietly be widened back. An explorer topic is a subject, not a slug."""
        from openstategraph.knowledge_builders import marker_source

        explorer_doc = (
            "<!-- generated by build-second-brain source=explorer hash=abc; "
            "regenerate via POST /api/workflows/<slug>/knowledge/build -->\n\nBody.\n"
        )

        assert marker_source(explorer_doc) == "explorer"
        assert marker_source(explorer_doc) not in ("", None, *self.POINTER_SOURCES)
