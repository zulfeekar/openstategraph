...
uv tool install \
  ...
  "openstategraph[server,ollama]==0.3.0rc11"
```
...
> **The published pre-release is older than this page, and the gap is on the
> half a stranger is most likely to want.** `0.3.0rc11` carries the editor, the
> compiler, the CLI and the MCP server. It does **not** carry the board
> (`openstategraph kanban`, `openstategraph patrol`) or the agent skills and
> config files that `init` writes — those landed after it was uploaded. Until
> the next pre-release, [With your coding agent](#with-your-coding-agent) and
> [The board](#the-board) below need the checkout route. Nothing else on this
> page depends on which one you have.
